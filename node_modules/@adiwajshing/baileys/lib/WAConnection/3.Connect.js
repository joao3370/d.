...
class WAConnection extends _1_Validation_1.WAConnection {
    ...
    async connect() {
        // se já estivermos conectados, lance um erro
        if (this.state !== 'close') {
            throw new Constants_1.BaileysError('não pode conectar quando estado=' + this.state, { status: 409 });
        }
    ...
    async connectInternal(options, delayMs) {
        ...
        const connect = () => new Promise((resolve, reject) => {
            ...
            // determinar se reconectar deve ser usado ou não
    ...
    onMessageRecieved(message) {
        var _a, _b;
        if (message[0] === '!') {
            // quando o primeiro caractere na mensagem é um '!', o servidor está enviando um quadro de pong
            ...
            /* Verifique se esta é uma resposta a uma mensagem que enviamos */
            anyTriggered = this.emit(`${DEF_TAG_PREFIX}${messageTag}`, json);
            /* Verifique se esta é uma resposta a uma mensagem que esperamos */
            const l0 = json[0] || '';
    ...
    /** Envia um pedido de manutenção de atividade a cada X segundos, atualiza o servidor e responde com o último visto */
    startKeepAliveRequest() {
        ...
        this.keepAliveReq = setInterval(() => {
            ...
            /*
                verifique se já faz uma quantidade suspeita de tempo desde que o servidor respondeu com nosso último visto
                pode ser que a rede esteja fora do ar
            */
            if (diff > Constants_1.KEEP_ALIVE_INTERVAL_MS + 5000)
                this.unexpectedDisconnect(Constants_1.DisconnectReason.lost);
            else if (this.conn)
                this.send('?,,'); // se estiver tudo bem, envie um pedido de manutenção de funcionamento
